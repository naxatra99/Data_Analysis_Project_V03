...
.append("g")
.attr("transform", "translate(-20," + "800" + ")rotate(-90)");
...
function filterDimensionsWithUndesiredNames(dimensions) {
  return dimensions.filter(function (dimension) {
    return !dimension.startsWith("Norm_") && !dimension.startsWith("MCA_") && !dimension.startsWith("model");
  });
}
...
function loadCSVAndPopulateDropdown() {
    d3.csv("/static/test_continuous_data.csv", function (error, data) {
        ...
        // Filter out dimensions with numerical values for dropdown
...
function setupParallelCoordinates(coffee) {
    ...
    x.domain(dimensions = d3.keys(coffee[0]).filter(function (dimension) {
        ...
        // return !isNaN(coffee[0][d]) && !d.startsWith("MCA_");
            return !coffee.every(function (d) {
                return isNaN(+d[dimension]);
            }) && !dimension.startsWith("MCA_") && !dimension.startsWith("Norm_model")&& !dimension.startsWith("model_")&& !dimension.startsWith("Norm_transmission")&& !dimension.startsWith("transmission_") && !dimension.startsWith("Norm_fuelType")&& !dimension.startsWith("fuelType_");
        }));
        // return coffee.every(function (d) {
        //     return !isNaN(+d[dimension]);
        //     console.log("dimension after filtering" + dimension);
        // }) && !dimension.startsWith("MCA_");
        // console.log("dimension after filtering" + dimension)
    // }));
